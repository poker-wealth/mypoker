...
/** Reputation an applicant must reach, per spec. */
export const MIN_REPUTATION_FOR_AGENT = 700;

export interface Eligibility {
  eligible: boolean;
  /** Why not, in words a player can act on. Empty when eligible. */
  reasons: string[];
  reputation: number;
  required: number;
...
/**
 * Whether a player could be approved as an agent.
 *
 * Advisory only — it decides nothing. Approval is an ops action through the
 * internal endpoint, per spec ("TG customer service contact workflow"), because
 * a self-service path would let anyone who cleared the numeric bar enrol
 * themselves and start taking a cut of the rake.
 *
 * Reputation is the gate, and a confirmed collusion finding disqualifies on its
 * own regardless of score — a colluder who ground their way back above 700
 * should not become an agent.
 */
export async function agentEligibility(
  playerId: string,
  reputation: { score: number },
  findings: { hasConfirmedCollusion: boolean; antiBotHighRisk: boolean },
): Promise<Eligibility> {
  const reasons: string[] = [];
  const alreadyAgent = (await AgentModel.findById(playerId).lean()) !== null;

  if (reputation.score < MIN_REPUTATION_FOR_AGENT) reasons.push('reputation_below_700');
  if (findings.hasConfirmedCollusion) reasons.push('confirmed_collusion');
  if (findings.antiBotHighRisk) reasons.push('anti_bot_high_risk');

  return {
    eligible: reasons.length === 0 && !alreadyAgent,
    reasons,
    reputation: reputation.score,
    required: MIN_REPUTATION_FOR_AGENT,
    alreadyAgent,
  };
}

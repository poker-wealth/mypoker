 ...
 *
 * Only ever binds a NEW player — one with zero rounds played. See the check
 * inline below for why, and why it throws rather than staying silent.
 */
export async function bindReferral(playerId: string, linkId: string): Promise<void> {
  ...
  // Owner's rule: the product's own pitch is "agents earn a share of the rake
  // from players they bring in." A player who has already played a hand was
  // not brought in by whoever handed them this link today — they arrived on
  // their own at some point in the past, and clicking a link now doesn't
  // rewrite that history. Binding them would divert house rake, generated by
  // a player nobody acquired, to an agent who did nothing.
  //
  // "Has played a hand" is the honest, enforceable line: it needs no
  // just-created-at-signup flag threaded through the gateway (more code, more
  // places for that flag to be stale or dropped), and it is unambiguous at
  // both ends — zero rounds is definitely new, any rounds played is
  // definitively not. This reuses getReputationFacts's roundsPlayed (itself
  // sourced from getPlayerStats().handsPlayed) rather than re-deriving a hand
  // count a second way, so this rule and reputation's "rounds played" cannot
  // quietly disagree.
  //
  // This throws rather than silently no-opping like the already-bound guard
  // above. That guard is silent specifically because a retried signup hitting
  // the same (playerId, linkId) twice is not an error — nothing is lost by
  // staying quiet about it. This is not a retry: it's a genuine rule
  // rejection, and a silent no-op here would be indistinguishable from
  // success to any future caller (a dashboard, an ops tool, a later refactor)
  // with no way to tell "bound" from "correctly refused". The gateway
  // currently swallows this error (console.warn, no toast), so the player
  // sees nothing different either way — but the distinction still needs to
  // exist somewhere for whoever reads the logs or writes the next caller.
  const { roundsPlayed } = await getReputationFacts(playerId);
  if (roundsPlayed > 0) {
    throw new AgentError('player has already played — not eligible for referral attribution');
  }

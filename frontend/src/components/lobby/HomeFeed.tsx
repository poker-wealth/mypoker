...
/**
 * The home feed — promo banners, CREATE / JOIN, All games, Tournament, and the
 * Live Tables screen JOIN opens.
 *
 * This WAS the Lobby tab. On 15 Sep 2026 the owner moved it to the top of
 * Alliance so the Lobby could go straight to Texas Hold'em (pages/Lobby.tsx).
 * It moved whole; where the comments below say "the lobby", they mean this feed.
 *
 * `children` render under the feed on its home view only. The host page's own
 * content (Alliance's lists) belongs beneath the feed, not beneath the Live
 * Tables screen that JOIN swaps in.
 */
export function HomeFeed({ children }: { children?: ReactNode }) {

...
export class NiuNiuRoom extends BaseLiveRoom<NiuNiuRoomConfig, RoomSeat> {
  ...
  protected handleAct(playerId: string, action: { type: string; amount?: number }): void {
    ...
    if (banker) {
      // Niu Niu max win multiplier is 5x (Bomb / Five Flowers Niu)
      this.checkBankerExposure(banker, amount, 5, seat.index);
      (seat as any).maxMultiplier = 5;
    }
  ...
  private async resolveRound(): Promise<void> {
    ...
    this.game.claimBanker(banker.playerId);

...
## The Android SDK path must not contain a space

The NDK bakes its own absolute path into the linker command and does not quote
it. On a Windows account like `C:\Users\MTECH COMPUTERS\`, the path splits and
the build dies with ~80 undefined C++ symbols — an error that reads as a broken
dependency and points nowhere near the cause. It cost most of a day to find.

`npm run android` now preflights this (`scripts/check-android-env.mjs`) and
prints the junction remedy. It cannot be fixed in the repo: `android/` is
generated by prebuild and gitignored, and `local.properties` is written from
`ANDROID_HOME` at build time — so the check is the durable form of the fix.

...
export function AllianceScreen() {
  ...
  const [membersFor, setMembersFor] = useState<League | null>(null);
  ...
  return (
    <>
                    ...
                    <LeagueRow
                      ...
                      action={
                        <View style={{ gap: 8, marginLeft: space.md, paddingVertical: space.xs }}>
                          ...
                          {/* The roster. On EVERY alliance you belong to, not just the one
                              you have entered — "who is in this one" is a question you ask
                              before entering, and the server scopes the answer to members
                              anyway (a non-member gets a 404). */}
                          <Pressable
                            onPress={() => setMembersFor(l)}
                            style={{ backgroundColor: '#2A2A2A', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 12, flexDirection: 'row', alignItems: 'center', gap: 6 }}
                          >
                            <Svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2}>
                            ...
                            <Text style={{ color: 'white', fontFamily: weight('700'), fontSize: 13 }}>
                              {t('alliance.roster')}
                            </Text>
                          </Pressable>
                        </View>
      ...
      <MembersSheet
        leagueId={membersFor?.leagueId ?? null}
        {...(membersFor?.name !== undefined ? { leagueName: membersFor.name } : {})}
